...
import pandas as pd
from src.db import get_documents_for_user

def display_invoice_list(username: str):
    """
    Wyświetla listę faktur dla danego użytkownika.
    
    Args:
        username: Nazwa użytkownika
    """
    # Pobieranie dokumentów
    results = get_documents_for_user(username)
    
    if not results:
        st.info("Brak faktur do wyświetlenia.")
        return
    
    # Przetwarzanie wyników - konwersja na jednolitą listę dokumentów
    all_documents = []
    for doc in results:
    ...
    if not df.empty:
        ...
        with col1:
            filter_number = st.text_input("Filtruj po numerze faktury")
        with col2:
            filter_vendor = st.text_input("Filtruj po dostawcy")
        with col3:
        ...
        st.write(f"Znaleziono {len(df_display)} faktur.")
        ...
        # Wyświetlenie tabeli
        if df_display.empty:
            st.info("Brak faktur spełniających kryteria filtrowania.")
        else:
            for i, row in df_display.iterrows():
                ...
                with col7:
                    ...
                    company = row['Firma']
                    if st.button("Szczegóły", key=f"details_{invoice_no}_{company}", type="primary", use_container_width=True):
